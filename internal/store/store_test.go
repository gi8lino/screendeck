...
func TestAdvanceRoundNarrowsMatches(t *testing.T) {
	...
	require.NoError(t, err)
	_, err = database.Vote(ctx, "ROUND1", "p1", "c", true)
	require.NoError(t, err)
	_, err = database.Vote(ctx, "ROUND1", "p2", "a", true)
	require.NoError(t, err)
	_, err = database.Vote(ctx, "ROUND1", "p2", "b", true)
	require.NoError(t, err)
	_, err = database.Vote(ctx, "ROUND1", "p2", "c", true)
	require.NoError(t, err)
	...
	_, err = database.Vote(ctx, "ROUND1", "p2", "b", true)
	require.NoError(t, err)
	_, err = database.Vote(ctx, "ROUND1", "p2", "c", false)
	require.NoError(t, err)
	...
	_, _, _, err = database.AdvanceRound(ctx, "ROUND1", "p1", 3)
	require.Error(t, err)
}
